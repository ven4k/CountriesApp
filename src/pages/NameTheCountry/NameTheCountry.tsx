...
export const NameTheCountry = () => {
    ...
    const filtered = filteredCountries.filter(el => (el.name.common.toLowerCase().includes(flagName.toLowerCase())) || el.translations.rus.common.toLowerCase().includes(flagName.toLowerCase()))
    ...
    const { array } = useMemo(() => {
        ...
        for (let i = 0; i < (filteredCountries.length - 1); ++i) {
            let random = Math.round(Math.random() * ((filteredCountries.length - 1) - i));
    ...
    return (
        <div className={styles.guessTheFlagPage}>
            ...
            <div className={styles.settingsBlock}>
                <div className={styles.open} onClick={() => setSettingsOpen(!settingsOpen)}>
                    <img className={styles.settingsIco} src={settingsIco} alt='settings ico' />
                    <span>Choose Region</span>
                </div>

                <div className={clsx({[styles.settings]:settingsOpen, [styles.hiddenSettings] : !settingsOpen })}>
                    <div className={styles.chooseRegion}>
                        <div className={styles.regionList}>
                            {uniqueRegion.sort().map((el) => (
                                <div key={el} >
                                    <input type='checkbox' checked={regionState.find(q => q.region === el)? true: false} id={el} className={styles.listItems} onChange={(e) => handleChooseClick(e, el as string)} />
                                    <label htmlFor={el}>{el}</label>
                                </div>
                            ))}
                             <button className={styles.saveBtn} disabled={lastRegionLength.length === currentRegionLength.length} onClick={handleSaveSettingsClick}>Save</button>
                        </div>
                    </div>
                </div>
            </div >
